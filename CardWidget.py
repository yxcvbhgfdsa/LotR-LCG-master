...
def remove_encounter_damage_markers_for_card(card, amount: int) -> int:
    """从遭遇卡的缓存状态中移除指定数量的 Damage 标记。"""
    key = marker_state_key_for_card(card)
    state = copy.deepcopy(_MARKER_STATE_CACHE.get(key, {}))
    if not isinstance(state, dict):
        return 0
    top_right = list(state.get("top_right", []) or [])
    remaining = max(0, int(amount or 0))
    removed = 0
    for index in range(len(top_right) - 1, -1, -1):
        if remaining <= 0:
            break
        if top_right[index] != "Damage":
            continue
        del top_right[index]
        remaining -= 1
        removed += 1
    state["top_right"] = top_right
...
class CardImageZoomDialog(QDialog):
    def __init__(
        ...
        *,
        ...
        if details is None:
            max_width = min(800, int(screen.width() * 0.8), content_limit.width())
            max_height = min(1000, int(screen.height() * 0.8), content_limit.height())
        else:
            max_width = min(
                max(1, int(screen.width() * 0.9) - 40),
                content_limit.width(),
            )
            max_height = min(
                max(1, int(screen.height() * 0.9) - 40),
        ...
        self.preview_widget = CardPreviewWidget(
            ...
            image_style="border: 2px solid #888;",
            parent=self,
        )
        ...
        self.setFixedSize(
            self.preview_widget.width() + 40,
            self.preview_widget.height() + 40,
        )
...
class CardWidget(QWidget):
    ...
    def show_zoomed_card(self):
        ...
        self.zoom_dialog = CardImageZoomDialog(
            self.current_pixmap,
            self,
            details=details,
        )
